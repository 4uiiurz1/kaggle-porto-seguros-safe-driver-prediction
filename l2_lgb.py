...
def train_predict(train_df, test_df, l1_models, params, model_name=None, use_feat=True):
    if model_name == None:
        #model_name = 'l2_lgb_%s'%datetime.now().strftime('%m%d%H%M')
        model_name = 'l2_lgb'
    log = Logger(os.path.join('log', '%s.log'%model_name))

    for l1_model in l1_models:
        train_df[l1_model] = pd.read_csv(os.path.join('local_cv', l1_model))['target']
        test_df[l1_model] = pd.read_csv(os.path.join('submission', l1_model))['target']
