...
//for PortoSeguro --Begin--
//took from https://www.csie.ntu.edu.tw/~cjlin/libsvmtools/eval/index.html
// only for auc
typedef std::vector<double> dvec_t;
typedef std::vector<int>    ivec_t;

class Comp{
	const ffm_double *dec_val;
	public:
	Comp(const ffm_double *ptr): dec_val(ptr){}
	bool operator()(ffm_int i, ffm_int j) const{
		return dec_val[i] > dec_val[j];
	}
};


ffm_double auc(const dvec_t& dec_values, const ivec_t& ty){
	ffm_double roc  = 0;
...
ffm_model ffm_train_on_disk(string tr_path, string va_path, ffm_parameter param) {
    ...
    if(!va_path.empty())
    {
        ...
        cout.width(13); //for PortoSeguro
        cout << "va_gini"; //for PortoSeguro
    ...
    for(ffm_int iter = 1; iter <= param.nr_iters; iter++) {
        ...
        if(!va.is_empty()) {
            ...
            ivec_t true_labels;
        		...
        		for(ffm_int ii = 0; ii < va.meta.l; ii++) {

                ffm_float y = va.Y[ii];

                ffm_node *begin = &va.X[va.P[ii]];

                ffm_node *end = &va.X[va.P[ii+1]];

                ffm_float r = param.normalization? va.R[ii] : 1;

                ffm_double t = wTx(begin, end, r, model);

            		ffm_double dec_value = 1/(1+exp(-t));

            		true_labels.push_back(y);
            		//if(y <= 0) dec_value *= -1;
            		dec_values.push_back(dec_value);

            		//cout << t << ' ' << y << ' ' << dec_value << endl;
            		}

            ffm_double va_auc = auc(dec_values, true_labels);
            ffm_double va_gini = 2.0 * va_auc - 1.0;
            //
            ...
            cout.width(13);
            cout << fixed << setprecision(5) << va_gini;

            if(auto_stop) {
                //if(va_loss > best_va_loss) {
                //if(va_loss > best_va_loss && va_gini < best_va_gini) { //replaced by
                if(va_gini < best_va_gini) { //replaced by
                    patience++;
